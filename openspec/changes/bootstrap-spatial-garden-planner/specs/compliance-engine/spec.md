...
The system SHALL attach a `sourceRuleId` to every non-success verdict so a reader can audit *which* rule produced the outcome. Rule definitions MUST live in a typed, reviewable data file (not inline in the evaluator) and MUST include a human-readable reference field.
