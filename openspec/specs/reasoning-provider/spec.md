...
1. A text field labelled "Anthropic API key" accepting free-text input with `autoCapitalize: AutoCapitalize.None`.
...
`apps/mobile/src/features/settings/hooks/use-anthropic-key.ts` SHALL export a `useAnthropicKey` hook returning `{ keyMasked: string | null, hasKey: boolean, saveKey: (plain: string) => Promise<void>, clearKey: () => Promise<void>, isLoading: boolean }`. The hook MUST read the current stored value from `expo-secure-store` on mount via tanstack-query and MUST invalidate the query on save / clear so the UI updates without a restart.
