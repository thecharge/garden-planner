# device-sqlite-adapter Specification

## Purpose

Defines the `expo-sqlite` adapter that wraps a `SQLiteDatabase` handle and satisfies the `SqliteLike` interface from `@garden/memory`, enabling the mobile app to use on-device SQLite for persistent storage.
