# capture-abort-on-blur Specification

## Purpose

Ensures that an in-progress `captureProtocol` scan is cleanly aborted when the Capture tab loses focus, releasing the DeviceMotion subscription and preventing sensor leaks.
